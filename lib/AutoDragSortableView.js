    ...
    moveTouch (nativeEvent,gestureState) {
        ...
        if (this.touchCurItem) {
            ...
            if (this.touchCurItem.moveToIndex != moveToIndex ) {
                ...
                shakedData.forEach((_item,index,array)=>{
                    let item = this.state.dataSource[_item.prevIndex]
                    let nextItem = null
                    if(_item.prevIndex!==this.touchCurItem.index) {
                        nextItem = this.state.dataSource[index]
                    }
                    if (nextItem != null) {
                        item.position.setValue({x: parseInt(nextItem.originLeft+0.5),y: parseInt(nextItem.originTop+0.5)})
                        //
                        // Animated.timing(
                        //     {
