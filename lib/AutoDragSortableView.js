...
export default class AutoDragSortableView extends Component {
    ...
    startTouch(touchIndex) {
        ...
        console.log(key, this.sortRefs.has(key))
    ...
    moveTouch(nativeEvent, gestureState) {
        ...
        if (this.touchCurItem) {
            ...
            if (this.touchCurItem.moveToIndex != moveToIndex) {
                ...
                let shakedData = this.state.dataSource.map((it, index) => ({it, prevIndex: index}))
                let movedItem = shakedData.splice(this.touchCurItem.index, 1)
                shakedData = [...shakedData.slice(0, this.touchCurItem.moveToIndex), ...movedItem, ...shakedData.slice(this.touchCurItem.moveToIndex)]
                this.state.dataSource.forEach((item, index, array) => {
                    let nextItem = null
                    if (index !== this.touchCurItem.index) {
                        if ((!(index < this.touchCurItem.index && index < this.touchCurItem.moveToIndex)) && (!(index > this.touchCurItem.index && index > this.touchCurItem.moveToIndex))) {
                            if (index > this.touchCurItem.index) {
    ...
    render() {
        return (
            <ScrollView
                ...
                style={this.props.scrollProps && this.props.scrollProps.style || styles.container} {...this.props.scrollProps}>
...
AutoDragSortableView.propTypes = {
    ...
    scrollProps: PropTypes.shape(ScrollView.propTypes)
}
